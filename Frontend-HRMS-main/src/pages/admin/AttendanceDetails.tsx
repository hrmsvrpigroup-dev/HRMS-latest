  ...
  useEffect(() => {
    const fetchDetails = async () => {
      try {
        setLoading(true)
        if (dateStr && type) {
          const data = await adminApi.getAttendanceDetails(dateStr, type)
          setEmployees(data)
        }
      } catch (err) {
  ...
  return (
    <>
      <style>{`
        ...
        /* Content area lifted above hero */
        .att-det-content { 
          padding: 0 32px 32px;
          margin-top: -32px;
              ...
              <table className="att-det-table">
                ...
                <tbody>
                  {filtered.map((emp, idx) => {
                    ...
                    // Cycle through gradient colours for avatars
                    const gradients = [
                      ...
                      <tr key={emp.id} className="att-det-tr">
                        ...
                        <td className="att-det-td">
                          <button
                            ...
                            onClick={() => navigate(`/admin/employees/${emp.id}/portfolio`)}
                          >
