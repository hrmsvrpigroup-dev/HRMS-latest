...
export default function SuperAdminDocuments() {
  ...
  const fetchDocuments = async () => {
    try {
      const res = await superAdminApi.getDocuments()
  ...
  const handleUploadSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    if (!uploadFile || !selectedCompanyId || !docType) return
    setIsUploading(true)
    try {
      const fd = new FormData()
      fd.append('file', uploadFile)
      fd.append('tenantId', selectedCompanyId)
      fd.append('type', docType)
      await superAdminApi.uploadDocument(fd)
  ...
  const handleFileChangeForReplace = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const fd = new FormData()
      fd.append('file', file)
      await superAdminApi.replaceDocument(replacingDocId, fd)
      fetchDocuments()
    } catch (err: any) {
  ...
  const handleDeleteClick = async (docId: string, docName: string) => {
    ...
    setLoading(true)
    try {
      await superAdminApi.deleteDocument(docId)
      fetchDocuments()
    } catch (err) {
  ...
  // Grouping logic: Ensure folder for every company
  const groupedData: Record<string, { company: Company; docs: TenantDocument[] }> = {}
  companies.forEach(company => {
    groupedData[company.id] = {
      company,
      docs: []
    }
  })
  ...
  documents.forEach(doc => {
    if (groupedData[doc.tenantId]) {
      groupedData[doc.tenantId].docs.push(doc)
    }
  })
  ...
  const filteredFolders = folders.filter(f => {
    return (
      f.company.name.toLowerCase().includes(term) ||
      f.company.subdomain.toLowerCase().includes(term)
    )
  })

  const activeFolder = activeCompanyId ? groupedData[activeCompanyId] : null
  ...
  return (
    <div className="page-container" style={{ padding: '30px', fontFamily: 'Inter, system-ui, sans-serif' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '32px' }}>
        <div>
          <h1 style={{ fontSize: '28px', fontWeight: '800', color: '#0f172a', margin: 0, letterSpacing: '-0.025em' }}>
            Company Document Explorer
          </h1>
          <p style={{ color: '#64748b', marginTop: '6px', fontSize: '15px' }}>
            {activeFolder 
              ? `Documents for ${activeFolder.company.name}` 
              : 'Physical folders automatically organized on disk by Company Name.'}
          </p>
        </div>
        <button
      ...
      <div style={{ display: 'flex', gap: '16px', alignItems: 'center', marginBottom: '24px', background: '#fff', padding: '16px 24px', borderRadius: '12px', border: '1px solid #e2e8f0', boxShadow: '0 1px 3px rgba(0,0,0,0.02)' }}>
        {activeFolder && (
          <button
            onClick={() => { setActiveCompanyId(null); setSearchTerm(''); }}
            style={{ 
              display: 'flex', 
              alignItems: 'center', 
              gap: '8px', 
        ...
        <div style={{ position: 'relative', flex: 1, maxWidth: '450px' }}>
          ...
          <input
            type="text"
            placeholder={activeFolder ? "Search documents in this folder..." : "Search folders by Company Name..."}
            value={searchTerm}
            ...
            <div>
              {filteredFolders.length === 0 ? (
                <div style={{ background: '#fff', borderRadius: '16px', padding: '60px 40px', textAlign: 'center', border: '1px solid #e2e8f0' }}>
                  ...
                  <p style={{ color: '#64748b', marginTop: '8px', fontSize: '14px' }}>No companies found matching the search query.</p>
                </div>
              ) : (
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: '24px' }}>
                  {filteredFolders.map(({ company, docs }) => (
                    <div
                      key={company.id}
                      onClick={() => { setActiveCompanyId(company.id); setSearchTerm(''); }}
                      style={{ 
                        background: '#fff', 
                        borderRadius: '16px', 
            ...
            <div style={{ background: '#fff', borderRadius: '16px', border: '1px solid #e2e8f0', boxShadow: '0 4px 20px rgba(0,0,0,0.03)', overflow: 'hidden' }}>
              <div style={{ background: '#f8fafc', padding: '20px 24px', borderBottom: '1px solid #e2e8f0', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                  ...
                  <span style={{ fontWeight: '700', color: '#0f172a', fontSize: '16px' }}>
                    {activeFolder.company.name} Folder
                  </span>
                </div>
              ...
              {activeDocsFiltered.length === 0 ? (
                <div style={{ padding: '80px 40px', textAlign: 'center' }}>
                  ...
                  <p style={{ color: '#94a3b8', marginTop: '6px', fontSize: '14px' }}>Super Admin can upload company credentials directly into this folder.</p>
                  <button
                    onClick={openUploadModal}
        ...
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(15,23,42,0.4)', backdropFilter: 'blur(4px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 50 }}>
          <div style={{ background: '#fff', padding: '28px', borderRadius: '16px', width: '100%', maxWidth: '440px', border: '1px solid #e2e8f0', boxShadow: '0 20px 25px -5px rgba(0, 0, 0, 0.1)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
              <h2 style={{ fontSize: '20px', fontWeight: '800', color: '#0f172a', margin: 0, letterSpacing: '-0.025em' }}>Upload Company Document</h2>
            ...
            <form onSubmit={handleUploadSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '18px' }}>
              <div>
                <label style={{ display: 'block', fontSize: '13px', fontWeight: '700', color: '#475569', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: '6px' }}>Target Company</label>
                <select 
              ...
              <div>
                ...
                <select value={docType} onChange={e => setDocType(e.target.value)} required style={{ width: '100%', padding: '12px', borderRadius: '8px', border: '1px solid #cbd5e1', fontSize: '14px' }}>
                  ...
                  <option value="Address Proof (Utility Bill / Lease Agreement)">Address Proof</option>
                  <option value="Bank Account Details">Bank Account Details</option>
                  <option value="Authorized Signatory ID Proof">Authorized Signatory ID Proof</option>
